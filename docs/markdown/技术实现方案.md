...
基于 **Turborepo** monorepo 架构，使用 **Next.js + TypeScript + React** 构建。
...
│   ├── web/                    # 主Web应用（Next.js）
│   ├── screen-test/            # 屏幕测试工具
│   ├── camera-analyzer/        # 相机分析工具
│   └── performance-monitor/    # 性能监控App（React Native）
├── packages/
│   ├── ui/                     # 共享UI组件库
│   ├── utils/                  # 工具函数库
│   ├── database/               # 数据库Schema和操作
│   ├── api-client/             # API客户端
│   └── config/                 # 共享配置
└── services/
    ├── api/                    # 后端API服务
...
interface ColorTestConfig {
  colors: string[] // 测试颜色列表
  duration: number // 每个颜色停留时间（秒）
  autoSwitch: boolean // 自动切换
  brightness: number // 屏幕亮度（0-100）
}
...
interface ResponseTimeTest {
  objectSpeed: number // 移动速度（px/frame）
  objectSize: number // 物体大小
...
class PWMTest {
  ...
  cameraDetectionGuide() {
    return {
      title: '使用相机检测PWM频闪',
      steps: ['1. 将手机屏幕亮度调至最低', '2. 打开另一台手机的相机', '3. 对准屏幕查看是否有条纹', '4. 有条纹说明使用PWM调光'],
    }
  }
}
```
...
export function BenchmarkChart({ devices }: { devices: Device[] }) {
  ...
  useEffect(() => {
    ...
    const option = {
      ...
      xAxis: {
        type: 'category',
        data: ['AnTuTu', 'GeekBench 单核', 'GeekBench 多核', '3DMark']
      },
      yAxis: { type: 'value' },
...
class ImageQualityAnalyzer {
  ...
  async analyzeColors(imageData: ImageData) {
    ...
    cv.calcHist(new cv.MatVector([hsv]), channels, mask, hist, histSize, ranges)
...
class BatteryMonitor {
  ...
  getChargingSpeed(): number {
    const chargingPoints = this.dataPoints.filter(p => p.charging).slice(-10) // 最近10个数据点
  ...
  predictRemainingTime(): number {
    const dischargingPoints = this.dataPoints.filter(p => !p.charging).slice(-20)
    ...
    return (currentLevel * 100) / speed // 分钟
  }
...
export async function POST(request: Request) {
  try {
    await limiter.check(request, 10) // 每分钟最多10次请求
...
这份技术文档涵盖了所有项目的详细实现方案，包括代码示例、数据库设计、API设计等。你可以基于这些方案直接开始开发！
